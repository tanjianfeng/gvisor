...
func (u *userMemPayload) Get(size int) ([]byte, *tcpip.Error) {
	if size > u.Size() {
		// TODO(...): unsafe truncation. Update Payload interface
		// to provide an int64 version?
		size = int(u.Size())
	}
	v := buffer.NewView(int(size))
	srcCopy := u.src.TakeFirst(size)
	if _, err := srcCopy.CopyIn(u.t, v); err != nil {
		return nil, tcpip.ErrInvalidPayloadSize
	}
	return v, nil
}
...
// DropFirst drops first N bytes from src.
func (u *userMemPayload) DropFirst(n int) {
	u.src = u.src.DropFirst(n)
}
